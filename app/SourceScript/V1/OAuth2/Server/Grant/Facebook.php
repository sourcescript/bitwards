<?php
namespace SourceScript\V1\OAuth2\Server\Grant;
 ...
 */
class Facebook implements GrantTypeInterface {
    ...
    public function completeFlow($inputParams = null)
    {
        ...
        $authParams = $this->authServer->getParam(array('client_id', 'client_secret', 'fb_id', 'first_name', 'last_name', 'email_address'), 'post', $inputParams);
        ...
        if (is_null($authParams['fb_id'])) {
            throw new Exception\ClientException(sprintf($this->authServer->getExceptionMessage('invalid_request'), 'fb_id'), 0);
        }

        // Check if user's username and password are correct
        $userId = call_user_func($this->getVerifyCredentialsCallback(), $authParams['fb_id'], $authParams['first_name'], $authParams['last_name'], $authParams['email_address']);
